    ...
    <div className="flex justify-between items-center relative w-full text-black text-opacity-60">
      ...
      <Drawer width="280" placement="right" visible={visible} onClose={() => setVisible(false)} closable={false}>
        ...
        <div>
          ...
          <div className="flex">
            {themeList.map((item) => (
              <span className="relative w-50 h-10 mr-4 rounded cursor-pointer" key={item.style} onClick={() => configStore.switchStyle(item.style)}>
                ...
                {configStore.themeStyle === item.style ? <CheckOutlined className="absolute right-2.5 bottom-2.5" style={{ color: configStore.theme.primaryColor }} /> : ''}
              </span>
            ))}
          </div>
        </div>
          ...
          <div className="flex">
            {colorList.map((item) => (
              <Tooltip title={item[configStore.locale + '_name']} color={item.color + 'B3'} key={item.color}>
                <span className="w-5 h-5 leading-5 text-center rounded-sm text-white mr-2 cursor-pointer" style={{ background: item.color }} onClick={() => configStore.switchColor(item.color)}>
